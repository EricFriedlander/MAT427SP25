...
bagged_model <- rand_forest(trees = 500, mtry = .cols()) |> 
  set_engine("ranger", important = "impurity") |> 
...
<!-- 2.    For $b = 1,\ldots, B$: -->
<!--     a.    Fit tree $\hat{f}_b$ with $d$ splits ($d+1$ leaves) using residuals $r$ as response and features $X$ as predictors. -->
...
<!-- ```{r} -->
<!-- gbm_grid <- grid_latin_hypercube(trees(range = c(20, 100)), -->
<!--                                   tree_depth(range = c(1, 15)), -->
<!--                                   learn_rate(range = c(-10,-1)), -->
<!--                                   min_n(range = c(2, 40)), size = 50) -->
<!-- voter_folds = vfold_cv(voter_train, v = 5, repeats = 1, strata = voter_category) -->
<!-- tuning_results <- tune_grid( -->
<!--   gbm_wf, -->
<!--   resamples= voter_folds, -->
<!--   grid = gbm_grid -->
