...
This data set contains information about 3000 NFL field goal attempts over three seasons. The column we will be interested in predicting is `Made`, which has value `1` or `0` indicating if the
field goal was made or missed. If you want explanations for the other variables,
you can look at the notes in the Excel file (the `.xlsx` file) by hovinging over the variable names.
...
::: {.callout-tip title="Question"}
What is proportion of field goals in your training set were made? What does this mean in context
of determining a baseline level of accuracy that we want to beat?
:::

# Logistic Regression and Regularization

We will now extend what we've learned about $L_1$-regularization to logistic regression! In the same way that LASSO adds an $L_1$-norm penalty to the *objective* function for Ordinary Least Squares regression, we can add an $L_1$-norm penalty to the objective function for logistic regression. Whereas in OLS, the objective function was the sum of squared errors, the objective function for Logistic Regression is the log-likelihood. Other than this the principle works the same way... by including a regularization term, you induce sparsity.
...
::: {.callout-tip title="Question"}
Fit your final model on the full training set. Assess both models performance on the test set.
:::

## Exercise 15 (Very long question)

::: {.callout-tip title="Question"}
Use grid search and cross-validation to find the best $k$ for a KNN classification model on this data. Fit the final model on the full training set and assess it's performance on the test set. How does it perform compared to the model from Exercise 14?
:::

## Exercise 16 (Practive Interview Question)
